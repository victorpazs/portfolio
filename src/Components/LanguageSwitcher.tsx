...
export default function LanguageSwitcher({
  ...
  const handleSwitchLanguage = (newLanguage: CurrentLanguage) => {
    setLoad(true);
    setTimeout(() => {
      setLoad(false);
  ...
  return (
    <div className={`flex gap-4 items-center`}>
      {load ? (
        ...
        >
          <img src={currentLanguage.icon} width={"25px"} />
          <div className="absolute inline-flex items-center justify-center w-5 h-5 text-xs rounded-full -top-2 -right-2">
            <img src="/swap.svg" width="16px" />
          </div>
        </button>
      )}
      <div
      ...
      >
        {languages
          .filter((lan) => lan.language !== currentLanguage.language)
          .map((lang: CurrentLanguage) => (
            <button
              title={lang.title}
              onClick={() => handleSwitchLanguage(lang)}
              className="relative "
            >
              <img src={lang.icon} width={"22px"} />
            </button>
          ))}
      </div>
    </div>
